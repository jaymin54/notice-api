...
export const searchNotice = async (req: Request, res: Response) => {
  try {
    ...
    if (!query) {
      return res.status(400).json({ status: commonStatus.failed, message: 'Search query parameter (q) is required.' });
    }

    const searchedNotices = await db.notice.findMany({
      where: {
        OR: [
          { description: { contains: query } },
          { event: { contains: query } },
    ...
    if (searchedNotices.length === 0) {
      return res.status(404).json({ status: commonStatus.not_found, message: commonMessages.NO_RESULTS_FOUND });
    }

    return res.json({ status: commonStatus.success, data: searchedNotices , meta: {
      totalPages,
      currentPage: page,
      limit: limit,
    },});
